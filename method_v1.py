...
def get_image_from_link(driver, url, timeframe, s_range, form_data, retries=3):
    ...
    try:
        ...
        try:
          desired_state = form_data.get("l", {}).get("value", False)
          ...
          if checkbox.is_selected() != desired_state:
            ...
            print("❌ Error updating Log Scale checkbox:", e)
...
async def main(Screener_url, Period, Range, form_data):
    ...
    if is_colab():
        ...
        **Date**: {current_datetime}
        """
        await send_pdf_with_markdown(874628419, output_pdf, markdown_message) # maulik bhai
        await send_pdf_with_markdown(1209634176, output_pdf, markdown_message) # maulik bhai new 
        return output_pdf
